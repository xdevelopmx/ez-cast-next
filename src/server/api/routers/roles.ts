...
export const RolesRouter = createTRPCRouter({
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (rol) {
        rol.casting.forEach((date) => {
          ...
          const end_day =
            (date.fecha_fin
              ? date.fecha_fin.getDate()
              : date.fecha_inicio.getDate()) + 1;
          const start_month = date.fecha_inicio.getMonth();
          const end_month = date.fecha_fin
            ? date.fecha_fin.getMonth()
            : date.fecha_inicio.getMonth();
          const start_year = date.fecha_inicio.getFullYear();
          const end_year = date.fecha_fin
            ? date.fecha_fin.getFullYear()
            : date.fecha_inicio.getFullYear();
  ...
  createAplicacionTalento: publicProcedure
    .input(
      z.object({
        id_rol: z.number(),
        id_talento: z.number(),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const getResponse = ApiResponses(
        "RolesRouter_createAplicacionTalento",
        lang
      );

      if (input.id_rol <= 0 || input.id_talento <= 0 || input.id_ubicacion <= 0)
        return null;
          ...
          if (cazatalentos) {
            await ctx.prisma.alertas.create({
              data: {
                ...
                visto: false,
                mensaje: getResponse("alerta_message")
                  .replace("[N1]", `${cazatalentos.nombre}`)
                  .replace("[N2]", `${cazatalentos.apellido}`)
                  .replace("[N3]", `${rol.proyecto.nombre}`),
              },
            });
          }
        }
  ...
  updateAplicacionesTalentoByIdRolAndIdTalento: publicProcedure
    .input(
      z.object({
        talentos: z.array(
          z.object({
  ...
  getAllAplicacionesTalentoByProyecto: publicProcedure
    .input(
      z.object({
    ...
    .query(async ({ input, ctx }) => {
      ...
      return await ctx.prisma.roles.findMany({
        ...
        include: {
          aplicaciones_por_talento: {
            where:
              input.estados_aplicaciones &&
              input.estados_aplicaciones.length > 0
                ? {
  ...
  getAllCompleteByProyecto: publicProcedure
    .input(z.number())
    .query(async ({ input, ctx }) => {
      if (input <= 0) return null;
      return await ctx.prisma.roles.findMany({
  ...
  deleteRolById: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (
        ctx.session &&
        ctx.session.user &&
        ctx.session.user.tipo_usuario === TipoUsuario.CAZATALENTOS
      ) {
      ...
      if (user && user.tipo_usuario === TipoUsuario.CAZATALENTOS) {
        ...
        if (rol) {
          if (
            rol.id_media_lineas &&
            (!input.lineas ||
              (input.lineas && input.lineas.id && input.lineas.id === 0))
          ) {
          ...
          if (
            rol.id_media_foto_referencia &&
            (!input.foto_referencia ||
              (input.foto_referencia &&
                input.foto_referencia.id &&
                input.foto_referencia.id === 0))
          ) {
          ...
          if (rol.selftape) {
            if (
              rol.selftape.id_media_lineas &&
              (!input.lineas_selftape ||
                (input.lineas_selftape &&
                  input.lineas_selftape.id &&
                  input.lineas_selftape.id === 0))
            ) {
  ...
  saveRol: publicProcedure
      ...
      z.object({
        ...
        compensaciones: z.object({
          ...
          sueldo: z
            ...
            .nullish(),
          compensaciones_no_monetarias: z
            .array(
              z.object({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.filtros_demograficos) {
        if (
          input.filtros_demograficos.rango_edad_inicio > 110 ||
          input.filtros_demograficos.rango_edad_inicio <= 0 ||
          input.filtros_demograficos.rango_edad_fin > 110 ||
          input.filtros_demograficos.rango_edad_fin <= 0
        ) {
          throw new TRPCError({
      ...
      if (input.descripcion_rol) {
        ...
        if (
          input.descripcion_rol.nsfw &&
          input.descripcion_rol.nsfw.descripcion.length > 500
        ) {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
      ...
      const compensaciones = await ctx.prisma.compensacionesPorRoles.upsert({
        ...
        update: {
          datos_adicionales:
            input.compensaciones.compensacion.datos_adicionales,
          suma_total_compensaciones_no_monetarias:
            input.compensaciones.compensacion
              .suma_total_compensaciones_no_monetarias,
        },
        create: {
          id_rol: rol.id,
      ...
      if (input.compensaciones.sueldo) {
        ...
        try {
          await ctx.prisma.sueldosPorRoles.delete({
            where: { id_comp_por_rol: compensaciones.id },
          });
        } catch (e) {}
      }
      try {
        await ctx.prisma.compNoMonetariasPorRoles.deleteMany({
          where: { id_comp_por_rol: compensaciones.id },
        });
      } catch (e) {}

      if (input.compensaciones.compensaciones_no_monetarias) {
        const saved_compensaciones_no_monetarias =
          await ctx.prisma.compNoMonetariasPorRoles.createMany({
            data: input.compensaciones.compensaciones_no_monetarias.map((c) => {
              const comp_no_mone: {
                id_compensacion: number;
                descripcion_compensacion: string;
                id_comp_por_rol: number;
              } = { ...c, id_comp_por_rol: compensaciones.id };
              return comp_no_mone;
            }),
          });
      ...
      if (input.filtros_demograficos) {
        const filtros_demograficos =
          await ctx.prisma.filtrosDemoPorRoles.upsert({
            where: {
              id_rol: rol.id,
            },
            update: {
              ...
              rango_edad_en_meses:
                input.filtros_demograficos.rango_edad_en_meses,
        ...
        try {
          await ctx.prisma.generosPorRoles.deleteMany({
            where: { id_filtro_demo_por_rol: filtros_demograficos.id },
          });
        } catch (e) {}

        if (input.filtros_demograficos.generos) {
          const saved_generos = await ctx.prisma.generosPorRoles.createMany({
            data: input.filtros_demograficos.generos.map((g) => {
              return {
                id_genero: g,
        ...
        try {
          await ctx.prisma.aparenciasEtnicasPorRoles.deleteMany({
            where: { id_filtro_demo_por_rol: filtros_demograficos.id },
          });
        } catch (e) {}
        ...
        if (input.filtros_demograficos.apariencias_etnias) {
          const saved_etnias =
            await ctx.prisma.aparenciasEtnicasPorRoles.createMany({
              data: input.filtros_demograficos.apariencias_etnias.map((e) => {
                return {
                  id_aparencia_etnica: e,
                  id_filtro_demo_por_rol: filtros_demograficos.id,
                };
              }),
            });
        ...
        try {
          await ctx.prisma.animalPorRoles.deleteMany({
            where: { id_filtro_demo_por_rol: filtros_demograficos.id },
          });
        } catch (e) {}
      ...
      await ctx.prisma.habilidadesPorRoles.deleteMany({
        where: { id_rol: rol.id },
      });
      ...
      const habilidades_seleccionadas_por_rol =
        await ctx.prisma.habilidadesSelecPorRoles.createMany({
          data: input.descripcion_rol.habilidades.map((h) => {
            return {
              id_habilidad: h,
              id_habilidades_por_rol: habilidades_por_rol.id,
            };
          }),
        });
      ...
      if (input.descripcion_rol.nsfw) {
        ...
        const nsfw_seleccionados_por_rol =
          await ctx.prisma.nSFWSeleccionadosPorRoles.createMany({
            data: input.descripcion_rol.nsfw.ids.map((id) => {
              return { id_nsfw: id, id_nsfw_por_rol: nsfw_por_rol.id };
            }),
          });
      ...
      // CASTING
      try {
        await ctx.prisma.castingPorRoles.deleteMany({
          where: { id_rol: rol.id },
        });
      } catch (e) {}
      ...
      // FILMACIONES
      try {
        await ctx.prisma.filmacionPorRoles.deleteMany({
          where: { id_rol: rol.id },
        });
      } catch (e) {}

      const saved_fechas_filmaciones =
        await ctx.prisma.filmacionPorRoles.createMany({
      ...
      const requisitos = await ctx.prisma.requisitosPorRoles.upsert({
        ...
        update: {
          ...
          id_estado_republica:
            input.requisitos.id_estado_donde_aceptan_solicitudes,
        },
        create: {
          presentacion_solicitud: new Date(input.requisitos.fecha_presentacion),
          informacion: input.requisitos.info_trabajo,
      ...
      const medios_multimedia_por_rol =
        await ctx.prisma.mediosMultimediaPorRoles.findMany({
          where: {
      ...
      const saved_medios_multimedia_por_rol =
        await ctx.prisma.mediosMultimediaPorRoles.createMany({
          data: input.requisitos.medios_multimedia_a_incluir.map((m) => {
            return {
              id_requisitos_por_roles: requisitos.id,
              id_medio_multimedia: m,
            };
          }),
        });
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const compensaciones = await ctx.prisma.compensacionesPorRoles.upsert({
        ...
        update: {
          ...
          suma_total_compensaciones_no_monetarias:
            input.compensacion.suma_total_compensaciones_no_monetarias,
        },
        create: {
          id_rol: input.id_rol,
          datos_adicionales: input.compensacion.datos_adicionales,
  ...
  saveFiltrosDemograficos: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (
        input.rango_edad_inicio > 110 ||
        input.rango_edad_inicio <= 0 ||
        input.rango_edad_fin > 110 ||
        input.rango_edad_fin <= 0
      ) {
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: getResponse("invalid_age"),
      ...
      if (input.apariencias_etnias) {
        const saved_etnias =
          await ctx.prisma.aparenciasEtnicasPorRoles.createMany({
            data: input.apariencias_etnias.map((e) => {
              return {
                id_aparencia_etnica: e,
                id_filtro_demo_por_rol: filtros_demograficos.id,
              };
      ...
      const deleted_animal = await ctx.prisma.animalPorRoles.deleteMany({
        where: { id_filtro_demo_por_rol: filtros_demograficos.id },
      });
      if (!deleted_animal) {
        throw new TRPCError({
  ...
  saveDescripcionRol: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (rol.nsfw && rol.nsfw.nsfw_seleccionados.length > 0) {
        const deleted_nsfw = await ctx.prisma.nSFWPorRoles.delete({
          where: { id_rol: rol.id },
        });
  ...
  saveInfoCastingYFilmacion: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.action === "casting") {
        const deleted_fechas = await ctx.prisma.castingPorRoles.deleteMany({
          where: { id_rol: input.id_rol },
        });
        if (!deleted_fechas) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
        ...
        const deleted_fechas = await ctx.prisma.filmacionPorRoles.deleteMany({
          where: { id_rol: input.id_rol },
        });
        if (!deleted_fechas) {
          throw new TRPCError({
  ...
  saveRequisitosRol: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (medios_multimedia_por_rol.length > 0) {
        const deleted_medios_multimedia_por_rol =
          await ctx.prisma.mediosMultimediaPorRoles.deleteMany({
            where: {
              id_requisitos_por_roles: requisitos.id,
            },
      ...
      const saved_medios_multimedia_por_rol =
        await ctx.prisma.mediosMultimediaPorRoles.createMany({
          data: input.medios_multimedia_a_incluir.map((m) => {
            return {
              id_requisitos_por_roles: requisitos.id,
              id_medio_multimedia: m,
            };
            ...
            const result = roles
              .map((r) => {
                ...
                if (r.filtros_demograficos) {
                  ...
                  r.filtros_demograficos.generos
                    .map((g) => g.id_genero)
                    .forEach((id_genero) => {
                      if (
                        !genero_found &&
                        input.id_generos_rol.includes(id_genero)
                      ) {
                  ...
                  if (!r.filtros_demograficos.rango_edad_en_meses) {
                    if (
                      r.filtros_demograficos.rango_edad_inicio >=
                        input.edad_inicio &&
                      r.filtros_demograficos.rango_edad_fin <= input.edad_fin
                    ) {
                      porcentaje_filter += 13;
                    }
                  }
                  let apariencias_etnias_found = false;
                  r.filtros_demograficos.aparencias_etnicas
                    .map((a) => a.id_aparencia_etnica)
                    .forEach((id_etnia) => {
                      if (
                        !apariencias_etnias_found &&
                        input.id_apariencias_etnicas.includes(id_etnia)
                      ) {
                  ...
                  if (
                    r.filtros_demograficos.id_pais ===
                    talento.filtros_aparencias?.id_pais
                  ) {
                    porcentaje_filter += 10;
                  }
                }
                if (talento.preferencias) {
                  if (
                    r.tipo_trabajos.filter((t) =>
                      talento.preferencias?.tipos_de_trabajo
                        .map((tp) => tp.id_tipo_de_trabajo)
                        .includes(t.id_tipo_trabajo)
                    ).length > 0
                  ) {
                    porcentaje_filter += 5;
                  }
                ...
                if (r.proyecto.tipo) {
                  if (
                    input.id_tipos_proyectos.includes(
                      r.proyecto.tipo.id_tipo_proyecto
                    )
                  ) {
                    porcentaje_filter += 5;
                  }
                ...
                if (
                  locacion_casting &&
                  talento.preferencias?.locaciones
                    .map((l) => l.id_estado_republica)
                    .includes(locacion_casting)
                ) {
                  porcentaje_filter += 12;
                }
                if (
                  r.color_cabello.id ===
                  talento.filtros_aparencias?.id_color_cabello
                ) {
                  porcentaje_filter += 5;
                }
                if (
                  r.color_ojos.id === talento.filtros_aparencias?.id_color_ojos
                ) {
                  porcentaje_filter += 5;
                }
                ...
                if (r.compensaciones) {
                  if (
                    input.id_preferencias_de_pago.includes(
                      r.compensaciones.sueldo ? 1 : 2
                    )
                  ) {
                    porcentaje_filter += 5;
                  }
                ...
                if (r.habilidades) {
                  ...
                  r.habilidades.habilidades_seleccionadas.forEach((h) => {
                    if (
                      talento.habilidades
                        .map((th) => th.id_habilidad)
                        .includes(h.id_habilidad)
                    ) {
                      found_habilidades += 1;
                    }
                  });
              ...
              .filter((r) => {
                console.log(
                  `el porcentaje del rol ${r.nombre} es : ${r.porcentaje_filter}`
                );
