...
const Proyecto: NextPage = () => {
  ...
  const initFiles = async () => {
    ...
    if (state.files.media.foto_portada) {
      const file = await FileManager.convertUrlToFile(
        state.files.media.foto_portada.url,
  ...
  const updateProyecto = api.proyectos.updateProyecto.useMutation({
    onSuccess: async (data) => {
      const files: { foto_portada: Media | null; archivo: Media | null } = {
        foto_portada: null,
        archivo: null,
      };
      ...
      if (state.files.archivo) {
        if (state.files.touched.archivo) {
          files_to_be_saved.push({
            path: `cazatalentos/${data.id_cazatalentos}/proyectos/${data.id}/archivo`,
            name: `${state.files.archivo.file.name}-${time}`,
            file: state.files.archivo.file,
            base64: state.files.archivo.base64,
          });
        } else {
          files.archivo = {
            id: state.files.archivo?.id ? state.files.archivo.id : 0,
            nombre: state.files.archivo?.name,
            type: state.files.archivo?.file.type
              ? state.files.archivo.file.type
              : "",
      ...
      if (state.files.foto_portada) {
        if (state.files.touched.foto_portada) {
          files_to_be_saved.push({
            path: `cazatalentos/${data.id_cazatalentos}/proyectos/${data.id}/foto-portada`,
            name: `${state.files.foto_portada.file.name}-${time}`,
            file: state.files.foto_portada.file,
            base64: state.files.foto_portada.base64,
          });
        } else {
          files.foto_portada = {
            id: state.files.foto_portada?.id ? state.files.foto_portada.id : 0,
            nombre: state.files.foto_portada.name,
            type: state.files.foto_portada?.file.type
              ? state.files.foto_portada.file.type
              : "",
            url: state.files.foto_portada.url
              ? state.files.foto_portada.url
              : "",
    ...
    if (!state.hasErrors) {
      updateProyecto.mutate({
        ...
        proyecto: {
          ...
          director_casting: state.director_casting
            ? state.director_casting
            : "",
          telefono_contacto: state.telefono_contacto
      ...
      dispatch({
        ...
        value: {
          ...
          telefono_contacto: state.telefono_contacto
            ...
            : "",
          email_contacto: state.email_contacto ? state.email_contacto : "",
          email_contacto_confirmacion: state.email_contacto_confirmacion
  ...
  useEffect(() => {
    if (state.hasErrors != null) {
      ...
      dispatch({
        type: "update-proyecto-form",
        value: { errors: result.errors, hasErrors: result.hasErrors },
      });
    } else {
  ...
  const equipo_creativo = useMemo(() => {
    ...
    state.productor,
    state.casa_productora,
    state.director,
    state.agencia_publicidad,
  ]);
          ...
          >
            ...
            <div className="container_box_header">
              ...
              <div className="d-flex" style={{ margin: '-30px 0 50px 0' }}>
                ...
                <div>
                  <p
                    style={{ marginLeft: 20, fontSize: "1.7rem" }}
                    className="color_a h4 font-weight-bold mb-0"
                  >
                    <b>{proyecto.data?.nombre ?? textos["n_p"]}</b>
                  </p>
                  <p
                    style={{ marginLeft: 20, fontSize: "1.3rem" }}
                    className="mb-0"
                  >
                    <b>{textos["comencemos"]}</b>
                  </p>
                </div>
              </div>
