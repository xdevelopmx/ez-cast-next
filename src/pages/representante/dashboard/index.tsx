...
import { Box, Button, Dialog, DialogContent, Divider, Grid, Modal, Skeleton, Typography } from '@mui/material'
...
const DashboardPage: NextPage<DashboardRepresentante> = ({ user }) => {
    ...
    const saveProfileImage = async () => {
        if (new_profile_img) {
            ...
            const to_be_saved: {path: string, name: string, file: File, base64: string}[] = [{path: `representantes/${user.id}/foto-perfil`, name: `foto-perfil-representante-${user.id}-${time}`, file: new_profile_img, base64: base64_img}];
                        ...
                        <div className="container_box_header">
                            ...
                            <div className="d-flex">
                                <p className="color_a h4 font-weight-bold mb-0">
                                    ...
                                    {!info.isFetching && <b>Bienvenido, {info.data?.nombre} {info.data?.apellido}</b>}
                            ...
                            <Grid container>
                                <Grid item xs={12} md={5} sx={{ paddingTop: 3 }}>
                                    {info.isFetching && <Skeleton width={300}/>}
                                    {!info.isFetching && <Typography fontWeight={900} sx={{ fontSize: '1.4rem' }}>{info.data?.nombre} {info.data?.apellido}</Typography>}
                                    <div style={{ position: 'relative', width: 350, aspectRatio: '350/420', maxWidth: '100%' }}>
                                        <Image
                                            fill
                                            src={(info.data?.foto_perfil) ? info.data.foto_perfil.url : '/assets/img/no-user-image.png'}
                                ...
                                <Grid item xs={12} md={7} sx={{ paddingTop: 8 }}>
                                    <MContainer className="ml-5" direction="vertical">
                                        ...
                                        <MContainer className={`m-1`} direction="horizontal">
                                            ...
                                            {!info.isFetching && <Typography fontSize={'1.2rem'} fontWeight={300} variant="body1">{`Unión: ${(info.data?.info_basica?.union?.id_union === 99) ? info.data?.info_basica?.union?.descripcion : info.data?.info_basica?.union?.union.es}`}</Typography>}
                                        </MContainer>
                                        <MContainer className={`m-1`} direction="horizontal" styles={{ alignItems: 'center' }}>
                                            ...
                                            {info.isFetching && <Skeleton width={200}/>}
                                            {!info.isFetching && <Typography fontSize={'1.2rem'} fontWeight={300} variant="body1">{info.data?.info_basica?.estado_republica.es}</Typography>}
                                        </MContainer>
                                        ...
                                        <MContainer className={`m-1`} direction="horizontal" styles={{ alignItems: 'center' }}>
                                            ...
                                            {info.isFetching && <Skeleton width={200}/>}
                                            {!info.isFetching && <Typography fontSize={'1.2rem'} fontWeight={300} variant="body1">{(redes_sociales['pagina_web']) ? redes_sociales['pagina_web'] : 'N/A'}</Typography>}
                            ...
                            <Grid container mt={4} xs={12}>
                                <Grid xs={12}>
                                    ...
                                    {info.isFetching && <Skeleton width={200}/>}
                                    {!info.isFetching && <Typography>{info.data?.biografia}</Typography>}
                                </Grid>
                                <Grid xs={12} mt={4}>
                                    <Divider />
                                </Grid>
