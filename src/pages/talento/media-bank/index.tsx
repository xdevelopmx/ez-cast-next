...
export const MediaBank = (props: {
  ...
  const [selftape, setSelftape] = useState<{
    ...
    public: boolean;
    id: null | number;
  }>({ video: null, nombre: "", public: true, id: null });
  ...
  const saveSelftapeMedia = api.talentos.saveSelftape.useMutation({
    async onSuccess() {
      ...
      await media_por_talento.refetch();
      setSelftape({ video: null, nombre: "", public: true, id: null });
    ...
    return (
      <Box marginRight={1} display={"flex"} flexDirection={"column"}>
        <Button
          ...
          size="small"
          variant={
            props.selftape.url === current_selftape_video_url
              ? "contained"
              : "outlined"
          }
          endIcon={
        ...
        >
            ...
            onClick={() => {
              ...
              setConfirmationDialog({
                ...
                content: (
                  <Box>
                    <Typography>
                      Seguro que deseas eliminar este selftape?
                    </Typography>
                  </Box>
                ),
              });
          ...
          <Button
            // eslint-disable-next-line @typescript-eslint/no-misused-promises
            onClick={async () => {
              await navigator.clipboard.writeText(props.selftape.url);
      ...
      <MainLayout menuSiempreBlanco={true}>
        <AnimatePresence>
                      ...
                      >
                        ...
                        >
                          {!props.read_only && (
                            <Box display={"flex"} flexDirection={"row"} gap={2}>
                              <AddButton
                                text={`${textos["subir"] ?? ""} Self-Tape`}
                                aStyles={{
                                  margin: 10,
                                  backgroundColor:
                                    media && media.selftapes.length <= 5
                                      ? ""
                                      : "lightgrey",
                                }}
                                onClick={() => {
                          ...
                          {media && media.selftapes.length > 0 && (
                            <>
                              ...
                              >
                                <source
                                  src={current_selftape_video_url}
                                  type="video/mp4"
                                />
                                Lo sentimos tu navegador no soporta videos.
                              </video>
                              <MContainer
                                styles={{ marginTop: 16, width: "100%" }}
                                direction="horizontal"
                              >
                                <Box display="flex" flexDirection={"column"}>
                                  {media.selftapes.filter((s) => s.public)
                                    .length > 0 && (
                                    <>
          ...
          <Dialog
            ...
            onClose={() => {
              setDialogSeltape({ ...dialogSelftape, open: false });
            }}
          >
            <DialogContent>
              <Box>
                ...
                <Box ml={"calc(50% - 125px)"} my={2}>
                  <Button
                    style={{ fontWeight: 800, color: "#069cb1", width: 250 }}
                    ...
                    <input
                      ...
                      onChange={async (ev) => {
                        if (ev.target.files) {
                          ...
                          if (file) {
                            const base_64 =
                              await FileManager.convertFileToBase64(file);
                            setSelftape((prev) => {
                              return {
                                ...prev,
                                video: {
                  ...
                  if ((media && media.selftapes.length <= 5 && !selftape.id) || selftape.id) {
                    if (selftape.video) {
                      ...
                      const urls_saved = await FileManager.saveFiles([
                        {
                          path: `talentos/${props.id_talento}/videos`,
                          name: name,
                          file: selftape.video.file,
                          base64: selftape.video.base64,
                        },
                      ]);
                      ...
                      notify(
                        "warning",
                        `${textos["no_haz_seleccionado_ningun_video"] ?? ""}`
                      );
                    }
                  } else {
