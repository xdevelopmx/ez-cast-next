...
export const FormGroup: FC<Props> = ({
  ...
  if (innerIcon) {
    input = (
      <Box style={{ maxWidth: style?.width, position: "relative" }}>
        {input}
  ...
  if (label) {
    label_element = (
      <label style={labelStyle} className={labelClassName} htmlFor={id}>
        {label}{" "}
        {textBlueLabel && (
          <span style={{ color: "#069CB1", paddingLeft: 2 }}>
            {textBlueLabel}
          </span>
        )}{" "}
        {tooltip}
  ...
  return (
    <div className="form-group" style={rootStyle}>
      ...
      {!loading && input}
      <MotionDiv
        show={error != null && show_error_message != null}
