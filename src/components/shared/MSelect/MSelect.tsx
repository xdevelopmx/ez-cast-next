...
export const MSelect: FC<Props> = ({
  ...
  if (options.length > 0) {
    ...
    if (option) {
      if (
        ...
        option.value.length !== parseInt(option.value).toString().length
      ) {
        default_value = " ";
      }
    }
  ...
  const _default_option = disable_default_option
    ? []
    : [
        default_option
          ? default_option
          : {
              value: default_value,
              label: textos["selecciona_una_opcion"]
                ? textos["selecciona_una_opcion"]
                : "Texto No Definido",
            },
      ];
  ...
  return (
    <FormControl sx={styleRoot}>
      ...
      {loading && (
        <Skeleton
          className={`form-control-sm text_custom ${
      ...
      {!loading && (
        <>
          {multiple && (
            <Box position={"relative"}>
              ...
              <Select
                ...
                onChange={onChange}
                MenuProps={{
                  classes: { paper: "select-children" },
                  style: styleMenuProps,
                }}
              >
                {options.map((e, index) => {
                  return (
                    <MenuItem key={index} value={e.value}>
                      <Checkbox
                        checked={(value as string[]).indexOf(e.value) > -1}
                      />
                      {e.label}
                    </MenuItem>
                  );
          ...
          {!multiple && (
            ...
            >
              {_default_option.concat(options).map((e, index) => (
                <MenuItem
                  style={
                    highlight_default_option && index === 0
                      ? { backgroundColor: "gray", color: "white" }
                      : e.value === value
                      ? { backgroundColor: "#069cb1", color: "white" }
                      : {}
                  }
                  key={index}
                  value={e.value}
          ...
          {inferiorBlueText && (
            <Typography fontSize={14} sx={{ color: "#069cb1" }}>
              {inferiorBlueText}
            </Typography>
          )}
        </>
