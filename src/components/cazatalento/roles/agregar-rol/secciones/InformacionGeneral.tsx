...
export const InformacionGeneralRol: FC<Props> = ({
  fetching,
  state,
  onFormChange,
}) => {
      ...
      <Grid item xs={12} md={6} mt={4}>
        <MContainer direction="vertical" styles={{ gap: 8 }}>
          <MRadioGroup
            ...
            direction="vertical"
            onChange={(e) => {
              const tipo_rol = !tipos_roles.data
                ? null
                : tipos_roles.data.filter(
                    (tr) =>
                      tr.tipo.toLowerCase() === e.target.value.toLowerCase()
                  )[0];

              onFormChange({
                rol_principal_secundario: e.target.value,
                id_tipo_rol:
                  e.target.value !== state.rol_principal_secundario
                    ? tipo_rol
                      ? tipo_rol.id
                      : 0
                    : state.id_tipo_rol,
              ...
              tipos_roles.data
                ? tipos_roles.data
                    .filter(
                      (tr) =>
                        tr.tipo.toLowerCase() ===
                        state.rol_principal_secundario.toLowerCase()
                    )
                    .map((s) => {
                      return {
                        value: s.id.toString(),
