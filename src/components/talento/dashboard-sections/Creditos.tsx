...
export const Creditos = (props: { id_talento: number; read_only: boolean }) => {
  ...
  return (
    <>
      <Grid id={`${textos['credito']}s`} container sx={{ marginTop: '-50px', padding: '120px 0 0 0'  }}>
        <Grid item xs={12}>
          <SectionTitle
            titleSx={{ fontSize: 26 }}
            dividerSx={{ borderTop: "2px solid #069CB1" }}
            title={
              textos["credito"] ? `${textos["credito"]}s` : "Texto No definido"
            }
            textButton={
            ...
            onClickButton={
              !props.read_only
                ? () => {
                    // eslint-disable-next-line @typescript-eslint/no-floating-promises
                    router.push(
                      `/talento/editar-perfil?step=3&id_talento=${props.id_talento}`
                    );
                  }
                : undefined
            }
                    ...
                    return {
                      ...
                      clip: !credito.media ? (
                        ...
                        >
                          ...
                          <Typography>{credito.media.nombre}</Typography>
                        </MContainer>
                      ),
                      destacado: (
                        <div
