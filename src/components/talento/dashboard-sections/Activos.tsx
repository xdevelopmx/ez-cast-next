...
export const Activos = (props: { id_talento: number, read_only: boolean }) => {
        ...
        if (data) {
            arrays = {
                ...
                vehiculos: data.vehiculos.map((v) => {
                    if (v.tipo_vehiculo) {
                        return { tipo: (ctx.lang === 'es') ? v.tipo_vehiculo.es : v.tipo_vehiculo.en, marca: v.marca, modelo: v.modelo, color: v.color, anio: `${v.anio}` };
                    }
                ...
                vestuarios: data.vestuario.map((v) => {
                    if (v.tipo_vestuario_especifico && v.tipo_vestuario_especifico.tipo_vestuario) {
                        return { tipo: (ctx.lang === 'es') ? v.tipo_vestuario_especifico.tipo_vestuario.es : v.tipo_vestuario_especifico.en, tipo_especifico: (ctx.lang === 'es') ? v.tipo_vestuario_especifico.es : v.tipo_vestuario_especifico.en, descripcion: v.descripcion }
                    }
                    return { tipo: 'Otro', tipo_especifico: 'N/A', descripcion: v.descripcion }
                }),
                props: data.props.map((p) => {
                    if (p.tipo_props) {
                        return { tipo: (ctx.lang === 'es') ? p.tipo_props.es : p.tipo_props.en, descripcion: p.descripcion }
                    }
                    return { tipo: 'N/D', descripcion: 'N/D' }
                }),
                equipos_deportivos: data.equipo_deportivo.map((e) => {
                    if (e.tipo_equipo_deportivo) {
                        return { tipo: (ctx.lang === 'es') ? e.tipo_equipo_deportivo.es : e.tipo_equipo_deportivo.en, descripcion: e.descripcion }
                    }
                    return { tipo: 'N/D', descripcion: 'N/D' }
                })
    ...
    return (
        <Grid id="activos" container sx={{ mt: 10 }}>
            <Grid item xs={12}>
                <SectionTitle title={textos['activos'] ? `${textos['activos']}` : 'Texto No definido'} textButton={textos['editar'] ? textos['editar'] : 'Texto No definido'} onClickButton={(!props.read_only) ? () => {
            ...
            <Grid item xs={12}>
                ...
                {!loading && <Typography my={2} fontSize={30} sx={{ color: '#069cb1' }} fontWeight={900}>{textos['mascotas'] ? textos['mascotas'] : 'Texto No definido'}</Typography>}
                <MTable
            ...
            <Grid item xs={12} my={2}>
                {loading && <Skeleton className={'md-skeleton'} />}
                {!loading && <Typography my={2} fontSize={30} sx={{ color: '#069cb1' }} fontWeight={900}> {textos['vehiculos'] ? textos['vehiculos'] : 'Texto No definido'}</Typography>}
                <MTable
                    backgroundColorHeader='#069cb1'
                    styleHeaderTableCell={{ padding: '5px !important' }}
                    columnsHeader={[
            ...
            <Grid item xs={12} my={2}>
                {loading && <Skeleton className={'md-skeleton'} />}
                {!loading && <Typography my={2} fontSize={30} sx={{ color: '#069cb1' }} fontWeight={900}>{textos['vestuarios'] ? textos['vestuarios'] : 'Texto No definido'}</Typography>}
                <MTable
                    backgroundColorHeader='#069cb1'
                    styleHeaderTableCell={{ padding: '5px !important' }}
                    columnsHeader={[
                        ...
                        <Typography key={3} sx={{ color: '#fff' }} fontSize={'1.2rem'} fontWeight={600} component={'p'}>
                            {textos['tipo_especifico'] ? textos['tipo_especifico'] : 'Texto No definido'}
                        </Typography>,
                        <Typography key={4} sx={{ color: '#fff' }} fontSize={'1.2rem'} fontWeight={600} component={'p'}>
            ...
            <Grid item xs={12} my={2}>
                {loading && <Skeleton className={'md-skeleton'} />}
                {!loading && <Typography my={2} fontSize={30} sx={{ color: '#069cb1' }} fontWeight={900}>{textos['props'] ? textos['props'] : 'Texto No definido'}</Typography>}
                <MTable
                    backgroundColorHeader='#069cb1'
                    styleHeaderTableCell={{ padding: '5px !important' }}
                    columnsHeader={[
            ...
            <Grid item xs={12} my={2}>
                {loading && <Skeleton className={'md-skeleton'} />}
                {!loading && <Typography my={2} fontSize={30} sx={{ color: '#069cb1' }} fontWeight={900}>{textos['equipo_deportivo'] ? textos['equipo_deportivo'] : 'Texto No definido'}</Typography>}
                <MTable
                    backgroundColorHeader='#069cb1'
                    styleHeaderTableCell={{ padding: '5px !important' }}
                    columnsHeader={[
