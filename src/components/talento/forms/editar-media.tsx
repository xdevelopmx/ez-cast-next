...
interface Props {
  ...
  loading: boolean;
  onFormChange: (input: {
    [id: string]: string | number | number[] | Archivo[];
  }) => void;
}

export const EditarMediaTalento: FC<Props> = ({
  onFormChange,
  state,
  loading,
}) => {
      ...
      <Grid item xs={12} md={7}>
        <MContainer direction="vertical">
          <MContainer
            ...
            styles={{ gap: 10, marginBottom: 5, alignItems: "center" }}
          >
          ...
          <Typography sx={{ color: "#069CB1", margin: "25px 5px 5px 0px" }}>
            {textos["elegir_foto_perfil"]
              ? textos["elegir_foto_perfil"]
              : "Texto No Definido"}
          </Typography>
          {!loading && (
              ...
              elements={state.fotos.map((foto, i) => {
                return {
                  ...
                  content: (
                    <div
                      style={{
                        border: i === 0 ? "1px solid #069CB1" : "none",
                        padding: "10px",
                      }}
                    >
                      <Image
            ...
            onChange={(files: File[]) => {
              const files_converted = Promise.all(
                files.map(async (f) => {
                  ...
                  const file_on_state = state.fotos.filter((foto) =>
                    FileManager.compareBase64Strings(base64, foto.base64)
                  );
                  if (file_on_state.length > 0) {
                    return {
                      ...
                      name: f.name,
                      file: f,
                      id: file_on_state[0]?.id,
                      url: file_on_state[0]?.url,
                    };
                  } else {
        ...
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <MContainer direction="vertical">
              <MContainer
                direction="horizontal"
                styles={{ gap: 10, alignItems: "center" }}
              >
                <span className={"badge"}>
                  {" "}
                  <Image
                  ...
                  text={
                    <>
                      <Typography fontSize={"12px"} fontWeight={800}>
                        {textos["media_videos_tooltip_titulo"]
                          ? textos["media_videos_tooltip_titulo"]
                          : "Texto No Definido"}
                      </Typography>
                      <Typography fontSize={"12px"} fontWeight={400}>
                        {textos["media_videos_tooltip_body"]
                          ? textos["media_videos_tooltip_body"]
                          : "Texto No Definido"}
                      </Typography>
                    </>
              ...
              <DragNDrop
                ...
                text_button={`${
                  textos["agregar"] ? textos["agregar"] : "Texto No Definido"
                } ${textos["video"] ? textos["video"] : "Texto No Definido"}`}
                onChange={(files: File[]) => {
                  const files_converted = Promise.all(
                    files.map(async (f) => {
                      const base64 = await FileManager.convertFileToBase64(f);
          ...
          <Grid item xs={12}>
            <MContainer direction="vertical">
              ...
              <DragNDrop
                ...
                text_button={`${
                  textos["agregar"] ? textos["agregar"] : "Texto No Definido"
                } ${textos["audio"] ? textos["audio"] : "Texto No Definido"}`}
                onChange={(files: File[]) => {
                  const files_converted = Promise.all(
                    files.map(async (f) => {
                      const base64 = await FileManager.convertFileToBase64(f);
