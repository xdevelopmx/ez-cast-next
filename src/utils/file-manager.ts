...
export const FileManager = {
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = () => {
                ...
                if (base.includes('data:image')) {
                    resolve(new Buffer(base.replace(/^data:image\/\w+;base64,/, ''), 'base64').toString());
                }
                if (base.includes('data:application')) {
                    resolve(new Buffer(base.replace(/^data:application\/\w+;base64,/, ''), 'base64').toString());
                }
                if (base.includes('data:video')) {
                    resolve(new Buffer(base.replace(/^data:video\/\w+;base64,/, ''), 'base64').toString());
                }
                if (base.includes('data:audio')) {
                    resolve(new Buffer(base.replace(/^data:audio\/\w+;base64,/, ''), 'base64').toString());
                }
    ...
    checkFilesRepeatedInArray: async (files: File[]) => {
        ...
        const buffers: string[] = await Promise.all(n_files.map(async (f): Promise<string> => {
            return new Promise((resolve, reject) => {
                const reader = new FileReader();
                reader.onload = () => {
                    const base = reader.result as string;
    ...
    compareBase64Strings: (base64_1: string, base64_2: string) => {
        const bases: string[] = [base64_1, base64_2].map(base => {
            if (base.includes('data:image')) {
                new Buffer(base.replace(/^data:image\/\w+;base64,/, ''), 'base64').toString();
            }
            if (base.includes('data:application')) {
                new Buffer(base.replace(/^data:application\/\w+;base64,/, ''), 'base64').toString();
            }
            if (base.includes('data:video')) {
                new Buffer(base.replace(/^data:video\/\w+;base64,/, ''), 'base64').toString();
            }
            if (base.includes('data:audio')) {
                new Buffer(base.replace(/^data:audio\/\w+;base64,/, ''), 'base64').toString();
            }
    ...
    deleteFiles: async (to_be_deleted: string[]) => {
        const results = await Promise.all(to_be_deleted.map(async (clave) => {
        ...
        const errors: (string | null)[] = results.filter(r => r.error != null).map(r => r.error);
    ...
    saveFiles: async (to_be_saved: {path: string, file: File, base64: string, name: string}[]) => {    
        let result: {[name: string]: {presigned_url: string | null, url: string | null, error: string | null}}[] = [];
        if (to_be_saved) {
            result = await Promise.all(to_be_saved.map(async (file) => {
